...
def affine_layer(hidden_dim, x):
    ...
    # _batch_size = x.get_shape()[0]
    _features = x.get_shape()[1]

    # x = x  <- input to the function
    W = tf.get_variable("W", shape=(_features, hidden_dim))
    # W = tf.get_variable("W", shape=(_features, hidden_dim), initializer=tf.contrib.layers.xavier_initializer())
...
def fully_connected_layers(hidden_dims, x):
    ...
    if len(hidden_dims) > 0:
        for dim in hidden_dims:
            with tf.variable_scope("dim_" + str(itr)):
                if itr == 0:
                    z_out = tf.nn.relu(affine_layer(dim, x))
                else:
                    z_out = tf.nn.relu(affine_layer(dim, z_out))
            itr += 1
    else:
        with tf.variable_scope("dim_" + str(itr)):
            # z_out = tf.nn.relu(affine_layer(1, x))
            z_out = tf.nn.relu(x)
    return z_out
...
def MakeLogits(x_ph, hidden_dims):
    ...
    y_squeezed = tf.squeeze(affine_layer(1, fully_connected_layers(hidden_dims, x_ph)), axis=[1])
...
def MakeLoss(logits, y_ph):
    ...
    '''
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=y_ph))
